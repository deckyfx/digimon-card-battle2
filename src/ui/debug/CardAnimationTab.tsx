...
export function CardAnimationTab() {
  ...
  return (
    <div class="debug-panel">
      ...
      >
        Powered by <strong style={{ color: "#88c0d0" }}>GSAP 3</strong> — spring
        easing, timelines, and GPU-accelerated transforms.
      </p>

      <div class="animation-grid">
        {/* ── CONTROLS PANEL ── */}
        <div
          class="debug-panel debug-panel-purple"
          style={{
            padding: "16px",
            display: "flex",
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              value={selectedIdx()}
              onChange={(e) =>
                setSelectedIdx(parseInt(e.currentTarget.value, 10))
              }
            >
              <For each={filteredCards()}>
                {(card, idx) => (
                  <option value={idx()}>
                    #{card.number} — {card.name} ({card.level})
                  </option>
                )}
              </For>
            </select>
          ...
          <div>
            ...
            >
              <button
                class="debug-btn anim-preset-btn"
                onClick={presetDraw}
                disabled={busy()}
              >
                ✋ Draw
          ...
          <div>
            ...
            >
              <label class="anim-label" style={{ margin: 0 }}>
                Manual Transforms
              </label>
              <button
                class="debug-btn"
          ...
          >
            <label class="anim-toggle">
              <input
                type="checkbox"
                checked={isFlipped()}
                onChange={(e) => setIsFlipped(e.currentTarget.checked)}
              />
